...
function modeltheme_btn_shortcode($params, $content) {
    ...
    $id_selector = 'btn_custom_'.uniqid();
      ...
      $inline_css_var = 'background: '.esc_attr($gradient_color_1).' !important; /* Old browsers */
    ...
    $content .= '<div class="'.$align.' modeltheme_button wow '.$animation.'">';
        $content .= '<a href="'.$btn_url.'" class="button-winona '.$btn_size.' '.$id_selector.'" style="color:'.$text_color.'; '.$inline_css_var.';">'.$btn_text.'</a>';
    $content .= '</div>';
    return $content;
}
