...
||-> xgymwp_redux
*/
function xgymwp_redux($redux_meta_name1,$redux_meta_name2 = ''){

    global  $xgymwp_redux;

    $html = '';
    if (isset($redux_meta_name1) && !empty($redux_meta_name2)) {
        $html = $xgymwp_redux[$redux_meta_name1][$redux_meta_name2];
    }elseif(isset($redux_meta_name1) && empty($redux_meta_name2)){
        $html = $xgymwp_redux[$redux_meta_name1];
    }
...
||-> xgymwp_setup
*/
function xgymwp_setup() {

    /*
     * Make theme available for translation.
     * Translations can be filed in the /languages/ directory.
     * If you're building a theme based on xgymwp, use a find and replace
     * to change 'xgymwp' to the name of your theme in all the template files
     */
    load_theme_textdomain( 'xgymwp', get_template_directory() . '/languages' );

    // This theme uses wp_nav_menu() in one location.
    register_nav_menus( array(
        'primary'  => esc_html__( 'Primary menu', 'xgymwp' )
    ) );
...
add_action( 'after_setup_theme', 'xgymwp_setup' );
...
*/
function xgymwp_widgets_init() {

    global  $xgymwp_redux;

    register_sidebar( array(
        'name'          => esc_attr__( 'Sidebar', 'xgymwp' ),
        'id'            => 'sidebar-1',
        'description'   => esc_attr__( 'Sidebar 1', 'xgymwp' ),
    ...
    if (!empty($xgymwp_redux['mt_dynamic_sidebars'])){
        foreach ($xgymwp_redux['mt_dynamic_sidebars'] as &$value) {
            ...
            if ($id_lowercase) {
                register_sidebar( array(
                    'name'          => $value,
                    'id'            => $id_lowercase,
                    'description'   => esc_attr__( 'Sidebar ', 'xgymwp' ) . $value,
                    'before_widget' => '<aside id="%1$s" class="widget %2$s">',
                    'after_widget'  => '</aside>',
                    'before_title'  => '<h1 class="widget-title">',
                    'after_title'   => '</h1>',
    ...
    // FOOTER ROW 1
    if (isset($xgymwp_redux['mt_footer_row_1_layout'])) {
        $footer_row_1 = $xgymwp_redux['mt_footer_row_1_layout'];
        $nr1 = array("1", "2", "3", "4", "5", "6");
        if (in_array($footer_row_1, $nr1)) {
            for ($i=1; $i <= $footer_row_1 ; $i++) { 
                register_sidebar( array(
                    'name'          => esc_attr__( 'Footer Row 1 - Sidebar ','xgymwp').$i,
                    'id'            => 'footer_row_1_'.$i,
                    'description'   => esc_attr__( 'Footer Row 1 - Sidebar ', 'xgymwp' ) . $i,
    ...
    // FOOTER ROW 2
    if (isset($xgymwp_redux['mt_footer_row_2_layout'])) {
        $footer_row_2 = $xgymwp_redux['mt_footer_row_2_layout'];
        $nr2 = array("1", "2", "3", "4", "5", "6");
        if (in_array($footer_row_2, $nr2)) {
            for ($i=1; $i <= $footer_row_2 ; $i++) { 
                register_sidebar( array(
                    'name'          => esc_attr__( 'Footer Row 2 - Sidebar ','xgymwp').$i,
                    'id'            => 'footer_row_2_'.$i,
                    'description'   => esc_attr__( 'Footer Row 2 - Sidebar ', 'xgymwp' ) . $i,
                    'before_widget' => '<aside id="%1$s" class="widget vc_column_vc_container %2$s">',
                    'after_widget'  => '</aside>',
                    'before_title'  => '<h1 class="widget-title">',
                    'after_title'   => '</h1>',
    ...
    // FOOTER ROW 3
    if (isset($xgymwp_redux['mt_footer_row_3_layout'])) {
        $footer_row_3 = $xgymwp_redux['mt_footer_row_3_layout'];
        $nr3 = array("1", "2", "3", "4", "5", "6");
        if (in_array($footer_row_3, $nr3)) {
            for ($i=1; $i <= $footer_row_3 ; $i++) { 
                register_sidebar( array(
                    'name'          => esc_attr__( 'Footer Row 3 - Sidebar ', 'xgymwp').$i,
                    'id'            => 'footer_row_3_'.$i,
                    'description'   => esc_attr__( 'Footer Row 3 - Sidebar ', 'xgymwp' ) . $i,
                    'before_widget' => '<aside id="%1$s" class="widget vc_column_vc_container %2$s">',
                    'after_widget'  => '</aside>',
                    'before_title'  => '<h1 class="widget-title">',
                    'after_title'   => '</h1>',
...
add_action( 'widgets_init', 'xgymwp_widgets_init' );


/**
||-> Enqueue scripts and styles.
*/
function xgymwp_scripts() {

    global  $xgymwp_redux;

    //STYLESHEETS
    wp_enqueue_style( "font-awesome", get_template_directory_uri()."/css/font-awesome.min.css" );
    wp_enqueue_style( "xgymwp-responsive", get_template_directory_uri()."/css/responsive.css" );
    wp_enqueue_style( "xgymwp-media-screens", get_template_directory_uri()."/css/media-screens.css" );
    wp_enqueue_style( "owl-carousel", get_template_directory_uri()."/css/owl.carousel.css" );
    wp_enqueue_style( "owl-theme", get_template_directory_uri()."/css/owl.theme.css" );
    wp_enqueue_style( "animate", get_template_directory_uri()."/css/animate.css" );
    wp_enqueue_style( "xgymwp-style", get_template_directory_uri()."/css/styles.css" );
    wp_enqueue_style( "xgymwp-mt-style", get_stylesheet_uri() );
    wp_enqueue_style( "xgymwp-header-style", get_template_directory_uri()."/css/styles-headers.css" );
    wp_enqueue_style( "xgymwp-footer-style", get_template_directory_uri()."/css/styles-footer.css" );
    ...
    wp_enqueue_style( "xgymwp-gutenberg-frontend", get_template_directory_uri()."/css/gutenberg-frontend.css" );
    ...
    wp_enqueue_script( "xgymwp-custom-js", get_template_directory_uri() . "/js/xgymwp-custom.js", array("jquery"), "1.0.0", true );
    if ( is_singular() && comments_open() && get_option( "thread_comments" ) ) {
        wp_enqueue_script( "comment-reply" );
    }
}
add_action( "wp_enqueue_scripts", "xgymwp_scripts" );


/**
||-> Enqueue admin css/js
*/
function xgymwp_enqueue_admin_scripts( $hook ) {
    // JS
    wp_enqueue_script( "xgymwp_admin_scripts", get_template_directory_uri().'/js/xgymwp-admin-scripts.js' , array( 'jquery' ) );
    wp_enqueue_script( "loaders", get_template_directory_uri().'/js/loaders.css.js' , array( 'jquery' ) );
    // CSS
    wp_enqueue_style( "xgymwp_admin_css", get_template_directory_uri().'/css/admin-style.css' );
    wp_enqueue_style( "loaders", get_template_directory_uri().'/css/loaders.css' );
}
add_action('admin_enqueue_scripts', 'xgymwp_enqueue_admin_scripts');


/**
||-> Enqueue css to js_composer
*/
add_action( 'vc_base_register_front_css', 'xgymwp_enqueue_front_css_foreever' );
function xgymwp_enqueue_front_css_foreever() {
...
*/
function xgymwp_register_fontawesome_to_redux() {
    wp_register_style( 'redux-font-awesome', get_template_directory_uri().'/css/font-awesome.min.css', array(), time(), 'all' );  
    wp_enqueue_style( 'redux-font-awesome' );
}
add_action( 'redux/page/redux_demo/enqueue', 'xgymwp_register_fontawesome_to_redux' );


/**
||-> Force Visual Composer to initialize as "built into the theme". This will hide certain tabs under the Settings->Visual Composer page
*/
add_action( 'vc_before_init', 'xgymwp_vcSetAsTheme' );
function xgymwp_vcSetAsTheme() {
...
*/
/* ========= RESIZE IMAGES ===================================== */
add_image_size( 'xgymwp_related_post_pic500x300', 500, 300, true );
add_image_size( 'xgymwp_post_pic700x450',         700, 450, true );
add_image_size( 'xgymwp_post_widget_pic100x100',  100, 100, true );
add_image_size( 'xgymwp_530x600',                 530, 600, true );
add_image_size( 'xgymwp_1150x400',                1150, 400, true );
add_image_size( 'xgymwp_1420x140',                1420, 140, true );
add_image_size( 'xgymwp_testimonials_150x150',    150, 150, true );
add_image_size( 'xgymwp_clients_160x90',          160, 90, true );
add_image_size( 'xgymwp_portfolio01_390x275',     390, 275, true );
add_image_size( 'xgymwp_blogpost01_1420x170',     1420, 170, true );
add_image_size( 'xgymwp_about_625x415',           625, 415, true );
add_image_size( 'xgymwp_about_415x525',           415, 525, true );
add_image_size( 'xgymwp_testimonials02_250x530',  250, 300, true );
...
||-> LIMIT POST CONTENT
*/
function xgymwp_excerpt_limit($string, $word_limit) {
...
||-> BREADCRUMBS
*/
function xgymwp_breadcrumb() {
    
    $delimiter = '';
    $html =  '';
    //text for the 'Home' link
    $name = esc_attr__("Home", "xgymwp");
    ...
    $currentAfter = '</li>';
        ...
        if (is_category()) {
            ...
            $html .= $currentBefore . esc_attr__('404 Not Found','xgymwp') . $currentAfter;
        }
        if (get_query_var('paged')) {
            if (is_home() || is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author())
                $html .= $currentBefore;
            $html .= esc_attr__('Page','xgymwp') . ' ' . get_query_var('paged');
            if (is_home() || is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author())
                $html .= $currentAfter;
        }
    }
...
||-> PAGINATION
*/
if ( ! function_exists( 'xgymwp_pagination' ) ) {
    function xgymwp_pagination($query = null) {
...
*/
function xgymwp_search_filter($query) {
    if ($query->is_search && !isset($_GET['post_type'])) {
        $query->set('post_type', 'post');
    }
    return $query;
}
add_filter('pre_get_posts','xgymwp_search_filter');
...
*/
function xgymwp_add_editor_styles() {
    add_editor_style( 'css/custom-editor-style.css' );
}
add_action( 'admin_init', 'xgymwp_add_editor_styles' );


/**
||-> FUNCTION: CUSTOM SEARCH FORM
*/
function xgymwp_custom_search_form(){
    ...
    $content .= '<div class="modeltheme-search">
                    <form method="GET" action="'.esc_url(home_url('/')).'">
                        <input class="search-input" placeholder="'.esc_attr__('Enter search term...', 'xgymwp').'" type="search" value="" name="s" id="search" />
...
if(function_exists( 'set_revslider_as_theme' )){
    add_action( 'init', 'xgymwp_disable_revslider_update_notices' );
    function xgymwp_disable_revslider_update_notices() {
