...
function xgymwp_li_love_link($love_text = null, $loved_text = null) {
	...
	if( is_user_logged_in() ) {
		...
		$love_count = xgymwp_li_get_love_count( $post->ID );
		if (empty($love_count)) {
			$love_count = 0;
		}
		...
		echo '<div class="love-it-wrapper">';
			...
			if( ! xgymwp_li_user_has_loved_post( $user_ID, get_the_ID() ) ) {
...
function xgymwp_li_display_love_link( $content ) {

	// $types = apply_filters( 'xgymwp_li_display_love_links_on', array() );
	$types = apply_filters( 'xgymwp_li_display_love_links_on', array( 'post' ) );

	if( is_singular( $types ) && is_user_logged_in() ) {
		$content .= xgymwp_li_love_link();
	}
	return $content;
}
add_filter( 'the_content', 'xgymwp_li_display_love_link', 100 );
...
function xgymwp_li_user_has_loved_post($user_id, $post_id) {
...
function xgymwp_li_store_loved_id_for_user($user_id, $post_id) {
	$loved = get_user_option('li_user_loves', $user_id);
	if(is_array($loved)) {
		$loved[] = $post_id;
	} else {
...
function xgymwp_li_mark_post_as_loved($post_id, $user_id) {
	...
	if(update_post_meta($post_id, '_li_love_count', $love_count)) {	
		...
		xgymwp_li_store_loved_id_for_user($user_id, $post_id);
		return true;
	}
	return false;
}
...
function xgymwp_li_get_love_count($post_id) {
	$love_count = get_post_meta($post_id, '_li_love_count', true);
	if($love_count)
		return $love_count;
	return 0;
...
function xgymwp_li_process_love() {
	if ( isset( $_POST['item_id'] ) && wp_verify_nonce($_POST['love_it_nonce'], 'love-it-nonce') ) {
		if(xgymwp_li_mark_post_as_loved($_POST['item_id'], $_POST['user_id'])) {
			echo esc_attr__('loved','xgymwp');
		} else {
			echo esc_attr__('failed','xgymwp');
		}
	}
	die();
}
add_action('wp_ajax_love_it', 'xgymwp_li_process_love');


/**
SCRIPTS
*/
function xgymwp_li_front_end_js() {
	if(is_user_logged_in()) {
		wp_localize_script( 'love-it', 'love_it_vars', 
			array( 
				'ajaxurl' => admin_url( 'admin-ajax.php' ),
				'nonce' => wp_create_nonce('love-it-nonce'),
				'already_loved_message' => esc_attr__('You have already loved this item.', 'xgymwp'),
				'error_message' => esc_attr__('Sorry, there was a problem processing your request.', 'xgymwp')
			) 
		);	
	}
}
add_action('wp_enqueue_scripts', 'xgymwp_li_front_end_js');
?>